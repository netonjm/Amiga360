...
char *ua_fs (const WCHAR *s, int defchar)
{
	...
	if (s == NULL)
		return NULL;
	dc = FALSE;
	len = WideCharToMultiByte (fscodepage, 0, s, -1, NULL, 0, &def, &dc);
	if (!len) {
	...
	d = xmalloc (char, len + 1);
	dc = FALSE;
	WideCharToMultiByte (fscodepage,0, s, -1, d, len, &def, &dc);
	if (dc) {
		def = 0;
		for (i = 0; i < len; i++) {
...
char *ua_fs_copy (char *dst, int maxlen, const TCHAR *src, int defchar)
{
	...
	if (src == NULL)
		return NULL;
	dc = FALSE;
	len = WideCharToMultiByte (fscodepage, 0, src, -1, dst, maxlen, &def, &dc);
	if (!len) {
		dst[0] = 0;
		return dst;
	}
	if (dc) {
		...
		for (i = 0; i < len; i++) {
			if (dst[i] == 0) {
				...
				WideCharToMultiByte (0, DWFLAGS | (defchar >= 0 ? 0 : 0), s2, -1, d2, 1, defchar >= 0 ? &def : NULL, defchar >= 0 ? &dc : NULL);
				dst[i] = d2[0];
			}
		}
	}
