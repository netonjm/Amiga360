...
static void subfunc (uae_u8 *data, int cnt)
{
	...
	if (subcodebufferinuse[subcodebufferoffsetw]) {
		...
		uae_sem_post (&sub_sem);
#ifdef CDTV_SUB_DEBUG
		write_log ("CDTV: subcode buffer overflow 1\n");
#endif
		return;
	}
...
#ifdef CDTV_SUB_DEBUG
			write_log ("CDTV: subcode buffer overflow 2\n");
#endif
			break;
		}
...
static int play_cdtrack (uae_u8 *p)
{
	...
	if (start_found == 0) {
		...
		cd_error = 1;
		activate_stch = 1;
		write_log ("PLAY CD AUDIO: illegal start track %d\n", track_start);
		return 0;
	}
	play_end = end;
	play_start = start;
...
static void cdrom_command_accepted (int size, uae_u8 *cdrom_command_input, int *cdrom_command_cnt_in)
{
	...
	for (i = 0; i < *cdrom_command_cnt_in; i++)
		_stprintf (tmp + i * 3, "%02X%c", cdrom_command_input[i], i < *cdrom_command_cnt_in - 1 ? '.' : ' ');
	write_log ("CD<-: %s\n", tmp);
	if (size > 0) {
		tmp[0] = 0;
		for (i = 0; i < size; i++)
			_stprintf (tmp + i * 3, "%02X%c", cdrom_command_output[i], i < size - 1 ? '.' : ' ');
		write_log ("CD->: %s\n", tmp);
	}
#endif
...
static void tp_bput (int addr, uae_u8 v)
{
	...
	if (addr != 1)
		write_log ("6525 write %x=%02X PC=%x %d\n", addr, v, M68K_GETPC, regs.s);
#endif
	switch (addr)
	{
	case 0:
...
#ifdef CDTV_SUB_DEBUG
			write_log ("CDTV: requested non-existing subchannel data!? %d\n", subcodeoffset);
#endif
			v = 0;
		} else {
...
#ifdef CDTV_DEBUG_6525
	if (addr < 7 && addr != 1)
		write_log ("6525 read %x=%02X PC=%x %d\n", addr, v, M68K_GETPC, regs.s);
#endif
	return v;
}
...
#ifdef CDTV_SUB_DEBUG
				if (subcodeoffset >= 0)
					write_log ("CDTV: frame interrupt, subchannel not empty! %d\n", subcodeoffset);
#endif
				subcodeoffset = -1;
...
static uae_u32 dmac_bget2 (uaecptr addr)
{
	...
	case 0xa1:
		...
		if (cdrom_command_cnt_out >= 0) {
			...
			write_log ("CDTV: command register read while empty\n");
			v = last_out;
		}
		break;
	case 0xe8:
