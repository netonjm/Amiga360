		...
		if (!driver_installed[j]) {
			for (i = 0; i < MAX_TOTAL_SCSI_DEVICES; i++) {
				if (device_func[i] == devicetable[j]) {
					...
					write_log ("%s driver installed, ok=%d\n", device_func[i]->name, ok);
					break;
				}
			}
		}
...
static int getunitinfo (int unitnum, int drive, unsigned int csu, int *isaudio)
{
	uae_u8 buffer[2048];
	struct device_info di;
	if (sys_command_info (unitnum, &di, 0)) {
		write_log ("Scanning drive %s: ", di.label);
...
fallback:
	device_func_init (SCSI_UNIT_IMAGE);
	if (!sys_command_open_internal (unitnum, "", csu)) {
		write_log ("image mounter failed to open as empty!?\n");
		return -1;
	}
	return unitnum;
}
...
int sys_command_open (int unitnum)
{
	int v;
	waspaused[unitnum] = 0;
	v = sys_command_open_internal (unitnum, currprefs.cdslots[unitnum].name[0] ? currprefs.cdslots[unitnum].name : NULL, CD_STANDARD_UNIT_DEFAULT);
	if (!v)
		return 0;
#ifdef RETROPLATFORM
	rp_cd_device_enable (unitnum, true);
...
static int audiostatus (int unitnum)
{
	...
	p = device_func[unitnum]->exec_in (unitnum, cmd, sizeof (cmd), 0);
	freesem (unitnum);
	if (!p)
		return 0;
	return p[1];
...
{
	struct device_info *di2;
	if (failunit (unitnum))
		return NULL;
	if (!getsem (unitnum))
		return 0;
	di2 = device_func[unitnum]->info (unitnum, di, quick);
...
static uae_u64 cmd_readx (int unitnum, uae_u8 *dataptr, int offset, int len)
{
	...
	if (!getsem (unitnum))
		return 0;
	v = device_func[unitnum]->read (unitnum, dataptr, offset, len);
	freesem (unitnum);
	if (v)
		return len;
	return 0;
	...
	*reply_len = *sense_len = 0;
	...
	case 0x1a: /* MODE SENSE(6) */
	{
		...
		if (!dbd) {
			if (nodisk (&di))
				goto nodisk;
			blocks = di.sectorspertrack * di.cylinders * di.trackspercylinder;
			p[-1] = 8;
			wl(p + 0, blocks);
	...
	case 0x43: // READ TOC
		...
		case 0x42: // READ SUB-CHANNEL
		{
			...
			if (subq && format == 1) {
				...
				reladdr = fromlongbcd (&buf[4 + 3]);
				absaddr = fromlongbcd (&buf[4 + 7]);
				if (!msf) {
		...
		case 0xba: // SCAN
		{
			...
			if (!sys_command_cd_toc (unitnum, &ttoc))
				goto readerr;
			toc = &ttoc;
			scan = (cmdbuf[1] & 0x10) ? -1 : 1;
			start = rl (cmdbuf + 1) & 0x00ffffff;
			end = scan > 0 ? toc->lastaddress : toc->toc[toc->first_track_offset].paddress;
			type = cmdbuf[9] >> 6;
			if (type == 1)
				start = lsn2msf (start);
			if (type == 3)
		...
		case 0x48: // PLAY AUDIO TRACK/INDEX
		{
			...
			if (strack < toc->first_track || strack > toc->last_track ||
				...
				strack > etrack)
				goto errreq;
			start = toc->toc[toc->first_track_offset + strack - 1].paddress;
			end = etrack == toc->last_track ? toc->lastaddress : toc->toc[toc->first_track_offset + etrack - 1 + 1].paddress;
			sys_command_cd_pause (unitnum, 0);
		...
		case 0xa9: // PLAY AUDIO TRACK RELATIVE (12)
		{
			...
			if (nodisk (&di))
				goto nodisk;
			len = cmd == 0xa9 ? rl (cmdbuf + 6) : rw (cmdbuf + 7);
			track = cmd == 0xa9 ? cmdbuf[10] : cmdbuf[6];
			if (track < di.toc.first_track || track > di.toc.last_track)
				goto errreq;
			start = di.toc.toc[di.toc.first_track_offset + track - 1].paddress;
			rel = rl (cmdbuf + 2);
			start += rel;
			end = start + len;
...
notdatatrack:
	...
	default:
err:
		write_log ("CDEMU: unsupported scsi command 0x%02X\n", cmdbuf[0]);
errreq:
		lr = -1;
		status = 2; /* CHECK CONDITION */
		s[0] = 0x70;
...
int sys_command_scsi_direct_native (int unitnum, struct amigascsi *as)
{
	...
	ret = device_func[unitnum]->exec_direct (unitnum, as);
