...
static TCHAR *cfgfile_subst_path2 (const TCHAR *path, const TCHAR *subst, const TCHAR *file)
{
	...
	if (_tcslen (path) > 0) {
		int l;
		TCHAR *p = xmalloc (TCHAR, _tcslen (file) + _tcslen (path) + 2);
		_tcscpy (p, path); 
		_tcscat (p, "\\");
		_tcscat (p, file);
		return p;
	...
	for (i = 0; i < MAX_PATHS; i++) {
		if (mp->path[i][0] && _tcscmp (mp->path[i], ".\\") != 0 && _tcscmp (mp->path[i], "./") != 0) {
			if (_tcsnicmp (mp->path[i], s, _tcslen (mp->path[i])) == 0) {
				return my_strdup (s + _tcslen (mp->path[i]) + 1);
			}
		}
	}
	return my_strdup (s);
...
void cfgfile_save_options (struct zfile *f, struct uae_prefs *p, int type)
{
	...
	if (p->chipset_refreshrate > 0)
		cfgfile_write (f, "chipset_refreshrate", "%lf", p->chipset_refreshrate);
...
static int cfgfile_parse_host (struct uae_prefs *p, TCHAR *option, TCHAR *value)
{
	...
	if (cfgfile_yesno (option, value, "use_debugger", 0)
	...
	if (cfgfile_string (option, value, "config_version", tmpbuf, sizeof (tmpbuf) / sizeof (TCHAR))) {
		...
		if (tmpp) {
			*tmpp++ = 0;
			tmpp2 = tmpp;
			cfgver = _tstol (tmpbuf) << 16;
			tmpp = _tcschr (tmpp, '.');
			if (tmpp) {
				*tmpp++ = 0;
				cfgver |= _tstol (tmpp2) << 8;
				cfgver |= _tstol (tmpp);
				p->config_version = cfgver;
			}
		}
		return 1;
	}
	...
	if (_tcscmp (option, "displaydata") == 0 || _tcscmp (option, "displaydata_pal") == 0 || _tcscmp (option, "displaydata_ntsc") == 0) {
	    ...
	    vert = -1;horiz = -1;lace = -1;ntsc = -1;framelength = -1;vsync = -1;
	    locked = 0;
	    rate = -1;
		...
		*tmpp++ = '\0';
		...
		if (_tcscmp (option, "filesystem2") == 0) {
			...
			*tmpp++ = 0;
			...
			strcpy(tmpaname,aname);

			tmpp = _tcschr(tmpaname, '\\');
			...
			strcpy(xboxDriveName,tmpaname);

			goto empty_fs;
			 						

#if 0
			err = XFileCacheInit(0,     // Flags
	                           0,     // MaxCacheSize
	                           5,     // HardwareThread
	                           0,     // ScratchBufferSize
	                           0);    // AppVersion

			strcpy(cachePath,root);
			strcat(cachePath,"\\*");
			err = XFileCachePreload(4,
	                        cachePath);
#endif
...
static int cfgfile_load_2 (struct uae_prefs *p, const TCHAR *filename, bool real, int *type)
{
	...
	//str = cfgfile_subst_path (" ", p->path_rom.path[0], p->romfile, sizeof (p->romfile) / sizeof (TCHAR));
	//_tcsncpy ( p->romfile, str, sizeof (p->romfile) / sizeof (TCHAR) - 1);
	 p->romfile[sizeof (p->romfile) / sizeof (TCHAR) - 1] = '\0';
	//free (str);
...
void default_prefs (struct uae_prefs *p, int type)
{
	...
	p->gfx_vresolution = VRES_DOUBLE;
	p->gfx_afullscreen = GFX_FULLSCREEN;
	p->gfx_pfullscreen = GFX_FULLSCREEN;
	...
	memset (&p->floppyslots[0], 0, sizeof (struct floppyslot));
	memset (&p->floppyslots[1], 0, sizeof (struct floppyslot));
	memset (&p->floppyslots[2], 0, sizeof (struct floppyslot));
	memset (&p->floppyslots[3], 0, sizeof (struct floppyslot));

	strcpy (p->floppyslots[0].df, "df0.adf");
	strcpy (p->floppyslots[1].df, "df1.adf");
	strcpy (p->floppyslots[2].df, "df2.adf");
	strcpy (p->floppyslots[3].df, "df3.adf");
	...
	_tcscpy (p->path_rom.path[0], "GAME:\\KickStart");
	_tcscpy (p->path_floppy.path[0], "GAME:\\Roms");
	_tcscpy (p->path_hardfile.path[0], "GAME:\\Harddisks");
	...
	default_file = NULL;
	f = zfile_fopen_empty (NULL, "GAME:\\Config\\configstore", 100000);
	if (f) {
		uaeconfig++;
