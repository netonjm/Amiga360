...
static __inline int mmu_match_ttr(uaecptr addr, bool super, bool data)
{
	int res;
...
static void mmu_dump_table(const char * label, uaecptr root_ptr)
{
	...
	for (root_idx = 0; root_idx < ROOT_TABLE_SIZE; root_idx++) {
		...
		D(bug("ROOT: %03d U=%d W=%d UDT=%02d\n", root_idx,
				root_des & 8 ? 1 : 0,
				root_des & 4 ? 1 : 0,
				root_des & 3
			  ));
...
restart:
	...
	if (write) {
		if (l->write_protect) {
			D(bug("MMU: write protected (via %s) %lx\n", l->tt ? "ttr" : "atc", addr));
			goto fail;
		}
		if (!l->modified)
			goto restart;
...
static uaecptr REGPARAM2 mmu_lookup_pagetable(uaecptr addr, bool super, bool write)
{
	...
	if ((desc & 2) == 0) {
		D(bug("MMU: invalid root descriptor for %lx\n", addr));
		return 0;
	}
	...
	desc = phys_get_long(desc_addr);
	if ((desc & 2) == 0) {
		D(bug("MMU: invalid ptr descriptor for %lx\n", addr));
		return 0;
	}
	wp |= desc;
	if ((desc & MMU_DES_USED) == 0)
	...
	if ((desc & 1) == 0) {
		D(bug("MMU: invalid page descriptor log=%08lx desc=%08lx @%08lx\n", addr, desc, desc_addr));
		return desc;
	}
