...
#define DEBUG_LOG(...) do ; while(0)
 ...
 __declspec(align(32)) static SDL_Surface *display;
 __declspec(align(32)) static SDL_Surface *screen;
...
static int y_size_table[MAX_SCREEN_MODES] = { 200, 240, 256, 400, 350, 480, 512, 600, 768,  864,  720, 1024 };
...
static int sdl_lock (struct vidbuf_description *gfxinfo)
{
    ...
    if (SDL_LockSurface (display) == 0) {
		...
		gfxinfo->rowbytes = (int)display->pitch;
...
void handle_events (void)
{
	...
	switch (rEvent.type) {
	    ...
	    case SDL_MOUSEBUTTONUP: {
			...
			state = (rEvent.type == SDL_MOUSEBUTTONDOWN);
			buttonno = -1;
...
void gfx_set_picasso_state (int on)
{
	...
	if (on) {
		...
		if (picasso_vidinfo.width > 0)
			current_width  = picasso_vidinfo.width;		
		if (picasso_vidinfo.height)
			current_height = picasso_vidinfo.height;		 
		graphics_subinit ();
    } else {
