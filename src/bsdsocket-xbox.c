 ...
 *			 2011	   Ported to WinsockX code by Lantus
 *
...
static void printSockAddr (struct sockaddr_in *in)
{
    ...
    DEBUG_LOG ("Port %d,",    htons (in->sin_port));
    DEBUG_LOG ("Address %s,", xbox_InetNtoa (in->sin_addr));
}
#else
...
static int copysockaddr_n2a (uae_u32 a_addr, const struct sockaddr_in *addr, unsigned int len)
{
    ...
    put_word (a_addr + 2,   htons(addr->sin_port));
    put_long (a_addr + 4,   htonl(addr->sin_addr.s_addr));
...
{	 
	...
	//ioctlsocket (sb->s, FIONBIO, (u_long *)&flags);
 	 
    if ((s = accept (sb->s, (struct sockaddr *)&addr, &hlen)) != INVALID_SOCKET) { 
		s2 = getsd (sb, s);	
		...
		bsdsocklib_setherrno (sb, EWOULDBLOCK);
		//errno = EWOULDBLOCK;
		return -1;
...
uae_u32 bsdthr_Recv_2 (SB)
{
    ...
    int i;
    if (sb->from == 0) {
		foo = recv (sb->s, (char *)sb->buf, sb->len, 0);
		...
		foo = recvfrom (sb->s, (char *)sb->buf, sb->len, 0, (struct sockaddr *)&addr, &l);
...
{
    if (sb->to == 0) {
		DEBUG_LOG ("Sending '%s' to server\n", (char *)sb->buf, errno);
		return send (sb->s, (char *)sb->buf, sb->len, 0);
    } else {
		struct sockaddr_in addr;
		int l = sizeof (struct sockaddr_in);
		copysockaddr_a2n (&addr, sb->to, sb->tolen);
		return sendto (sb->s, (char *)sb->buf, sb->len, 0, (struct sockaddr *)&addr, l);
    }
}
...
uae_u32 bsdthr_blockingstuff (uae_u32 (*tryfunc)(SB), SB)
{
	...
	ioctlsocket (sb->s, FIONBIO, (u_long *)&nonblock);
...
static void *bsdlib_threadfunc (void *arg)
{
	...
	switch (sb->action) {
		...
		{    
			...
			char			      *name_list[2];
			const char			  *name = (char *)get_real_address (sb->name);
			const int			   numaddr = 1;
			const int			   numaliases = 0;
			...
			{   
				...
				put_long (sb->hostent + 8, AF_INET);
				put_long (sb->hostent + 12, 4);
				put_long (sb->hostent + 16, sb->hostent + 24 + numaliases*4);
				put_long (sb->hostent + 20 + numaliases * 4, 0);
				put_long (sb->hostent + 24 + (numaliases) * 4, addmem (&aptr, name_list[0], 4));
			...
			else
			{
				...
				name_list[0] = (char *)&pDns->aina[0].S_un.S_addr;
				name_list[1] = NULL;
			...
			
		{
			...
			name_list[0] = (char *)addr.ina.s_addr;
			name_list[1] = NULL;
...
void host_setsockopt (SB, uae_u32 sd, uae_u32 level, uae_u32 optname, uae_u32 optval, uae_u32 optlen)
{
	...
	if (level == 0xffff && optname == 0x2001) {
		...
		sb->ftable[sd-1] = (sb->ftable[sd-1] & ~REP_ALL) | REP_ACCEPT;
		sb->resultval = 0;
	...
	else
	{
		...
		sb->resultval  = setsockopt (s, nativelevel, nativeoptname, (const char *)buf, optlen);
	}
...
uae_u32 host_getsockname (SB, uae_u32 sd, uae_u32 name, uae_u32 namelen)
{
    ...
    if (s != -1) {
		...
		XNetXnAddrToInAddr(&xna, &key, &addr.sin_addr);		 
		addr.sin_addr.s_addr = xna.ina.s_addr;
		copysockaddr_n2a (name, &addr , a_nl);

		if (len > 8)
			memset (get_real_address (name + 8), 0, len - 8);
		      ...
		      uae_u32 timeout, uae_u32 sigmp)
{
	...
	DEBUG_LOG ("WaitSelect preempted by signals  %d %d, nfds = %d \n", sigs ,  wssigs, nfds);
...
{	
    ...
    if ((s = socket (af,type,protocol)) == -1)  {
		...
		//if( setsockopt(s, SOL_SOCKET, 0x5802, (PCSTR)&bBroadcast, sizeof(BOOL) ) != 0 )//PATCHED!
		//{
		//  printf( "Failed to set socket to 5802, error\n");
		//}

		if( setsockopt(s, SOL_SOCKET, 0x5801, (PCSTR)&bBroadcast, sizeof(BOOL) ) != 0 )//PATCHED!
		{
		  DEBUG_LOG( "Failed to set socket to 5801, error\n");
		}
#endif
...
uae_u32 host_Inet_NtoA (TrapContext *context, SB, uae_u32 in)
{
    ...
    *(uae_u32 *)&ina = htonl (in);
    ...
    if ((addr = xbox_InetNtoa(ina)) != NULL) {
			 ...
			 uae_u32 optval, uae_u32 optlen)
{
	...
	r = getsockopt (s, nativelevel, nativeoptname,
		(char *)optval ? (char *)buf : NULL, optlen ? &len : NULL);
...
#	endif
	...
	case 0x8004667E: /* FIONBIO */
		...
		r = ioctlsocket (sock, FIONBIO, (u_long *)&flags);

	    return r;

	case 0x4004667F: /* FIONREAD */
	    r = ioctlsocket (sock, FIONREAD, (u_long *)&flags);
	    if (r >= 0) {
			put_long (arg, flags);
	    }
	    return r;
...
{
    return xbox_GetHostname ((char *)get_real_address (name), namelen);
}
...
{   
	...
	if (newsock != sb->sockabort[0]) {		 
		closesocket(sb->sockabort[0]);
	}	 	
		 
	if (newsock != INVALID_SOCKET)
		sb->sockabort[0] = newsock;


	unlocksigqueue();	

	bsdsocklib_seterrno (sb, 0); /* EBADF */
