...
static void allocate_expamem (void)
{
	...
	if (allocated_gfxmem != currprefs.gfxmem_size) {
		if (gfxmemory)
		{
			VirtualFree (gfxmemory,allocated_gfxmem,MEM_RELEASE );
		...
		if (allocated_gfxmem) {
			gfxmemory = (uae_u8*)VirtualAlloc((LPVOID)p96ram_start,allocated_gfxmem,MEM_COMMIT|MEM_RESERVE,PAGE_READWRITE);
