...
#include <xtl.h> // for 'struct timeval' definition
...
extern void addmode (struct MultiDisplay *md, int w, int h, int d, int rate, int nondx);
extern void cfgfile_write (struct zfile *f, const TCHAR *option, const TCHAR *format,...);
...
extern struct MultiDisplay *getdisplay (struct uae_prefs *p);
extern void target_save_options (FILE *f, const struct uae_prefs *p); 
extern int target_parse_option (struct uae_prefs *p, const char *option, const char *value);
extern void target_default_options (struct uae_prefs *p, int type);
